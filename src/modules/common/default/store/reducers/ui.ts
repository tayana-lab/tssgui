import { createSlice } from '@reduxjs/toolkit';
import {
  addWindowClass,
  calculateWindowSize,
  removeWindowClass,
} from '@app/modules/common/default/utils/helpers';
import {
  NAVBAR_DARK_VARIANTS,
  NAVBAR_LIGHT_VARIANTS,
  SIDEBAR_DARK_SKINS,
  SIDEBAR_LIGHT_SKINS,
  } from '@app/modules/common/default/themes/themes';
import config  from '@modules/conf/TssGui.json';

export interface UiState {
  screenSize: string;
  menuSidebarCollapsed: boolean;
  controlSidebarCollapsed: boolean;
  darkMode: boolean;
  blueMode: boolean;
  headerBorder: boolean;
  headerFixed: boolean;
  footerFixed: boolean;
  layoutBoxed: boolean;
  layoutFixed: boolean;
  menuItemFlat: boolean;
  menuChildIndent: boolean;
  navbarVariant: string;
  sidebarSkin: string;
}

const getLocalStorageItem = (key: string, defaultValue: string) => {
  return localStorage.getItem(key) || defaultValue;
};
const darkModeValue = localStorage.getItem("dark-mode");
var darkMode;
if(darkModeValue){
   darkMode=JSON.parse(darkModeValue);
}
 
var defaultTheme = config.DEFAULT_THEME == "blue" ? "blue-mode" : "light-mode";

if (localStorage.getItem("blue-mode") === null) {
  localStorage.setItem("body-theme", defaultTheme);
}

const initialState: UiState = {
  screenSize: calculateWindowSize(window.innerWidth),
  darkMode: darkMode,
  blueMode: localStorage.getItem("body-theme") === "blue-mode",
  navbarVariant: getLocalStorageItem("nav-theme","navbar-light"),
  sidebarSkin:  getLocalStorageItem("side-theme" ,"sidebar-light-primary"),
  menuSidebarCollapsed: false,
  controlSidebarCollapsed: true,
  headerBorder: false,
  headerFixed: false,
  footerFixed: false,
  layoutBoxed: false,
  menuItemFlat: false,
  menuChildIndent: false,
  layoutFixed: false,
  
};


//addWindowClass('layout-footer-fixed');

//removeWindowClass('layout-footer-fixed');

export const uiSlice = createSlice({
  name: 'ui',
  initialState,
  reducers: {
    toggleSidebarMenu: (state) => {
      state.menuSidebarCollapsed =!state.menuSidebarCollapsed;
    },
    toggleControlSidebar: (state) => {
      state.controlSidebarCollapsed = !state.controlSidebarCollapsed;
    },
    toggleHeaderBorder: (state) => {
      state.headerBorder = !state.headerBorder;
    },
    toggleHeaderFixed: (state) => {
      state.headerFixed = !state.headerFixed;
      if (state.headerFixed) {
        addWindowClass('layout-navbar-fixed');
      } else {
        removeWindowClass('layout-navbar-fixed');
      }
    },
    toggleFooterFixed: (state) => {
      state.footerFixed = !state.footerFixed;
      if (state.footerFixed) {
        addWindowClass('layout-footer-fixed');
      } else {
        removeWindowClass('layout-footer-fixed');
      }
    },
    toggleLayoutBoxed: (state) => {
      state.layoutBoxed = !state.layoutBoxed;
      if (state.layoutBoxed) {
        addWindowClass('layout-boxed');
      } else {
        removeWindowClass('layout-boxed');
      }
    },
    toggleLayoutFixed: (state) => {
      state.layoutFixed = !state.layoutFixed;
      if (state.layoutFixed) {
        removeWindowClass('layout-fixed');
      } else {
        addWindowClass('layout-fixed');
      }
    },
    toggleMenuItemFlat: (state) => {
      state.menuItemFlat = !state.menuItemFlat;
    },
    toggleMenuChildIndent: (state) => {
      state.menuChildIndent = !state.menuChildIndent;
    },
toggleDarkMode: (state) => {
      state.darkMode = !state.darkMode;
      if (state.darkMode) {
        state.blueMode = false;
        state.navbarVariant = NAVBAR_DARK_VARIANTS[0].value;
        localStorage.setItem('nav-theme', NAVBAR_DARK_VARIANTS[0].value);
        state.sidebarSkin = SIDEBAR_DARK_SKINS[0].value;
        localStorage.setItem('side-theme', SIDEBAR_DARK_SKINS[0].value);
        addWindowClass('dark-mode');
        removeWindowClass('blue-mode');
        localStorage.setItem('body-theme', 'dark-mode');
        localStorage.setItem('dark-mode', JSON.stringify(true));
        localStorage.setItem('blue-mode', JSON.stringify(false));
      } else {
        state.navbarVariant = NAVBAR_LIGHT_VARIANTS[0].value;
        localStorage.setItem('nav-theme', NAVBAR_LIGHT_VARIANTS[0].value);
        state.sidebarSkin = SIDEBAR_LIGHT_SKINS[0].value;
        localStorage.setItem('side-theme', SIDEBAR_LIGHT_SKINS[0].value);
        removeWindowClass('dark-mode');
        state.blueMode = false;
        localStorage.setItem('body-theme', 'light-mode');
        localStorage.setItem('dark-mode', JSON.stringify(false));
      }
    },
    toggleBlueMode: (state) => {
      state.blueMode = !state.blueMode;
      if (state.blueMode) {
        state.darkMode = false;
        state.navbarVariant = NAVBAR_LIGHT_VARIANTS[0].value;
        localStorage.setItem('nav-theme', NAVBAR_LIGHT_VARIANTS[0].value);
        state.sidebarSkin = SIDEBAR_LIGHT_SKINS[0].value;
        localStorage.setItem('side-theme', SIDEBAR_LIGHT_SKINS[0].value);
        addWindowClass('blue-mode');
        removeWindowClass('dark-mode');
        localStorage.setItem('body-theme', 'blue-mode');
        localStorage.setItem('blue-mode', JSON.stringify(true));
        localStorage.setItem('dark-mode', JSON.stringify(false));
      } else {
        state.blueMode = false;
        localStorage.setItem('body-theme', 'light-mode');
        localStorage.setItem('blue-mode', JSON.stringify(false));
        removeWindowClass('blue-mode');
      }
    },
    setNavbarVariant: (state, { payload }) => {
      if (state.darkMode) {
        state.navbarVariant = payload || NAVBAR_DARK_VARIANTS[0].value;
      } else {
        state.navbarVariant = payload || NAVBAR_LIGHT_VARIANTS[0].value;
      }
    },
    setSidebarSkin: (state, { payload }) => {
      if (state.darkMode) {
        state.sidebarSkin = payload || SIDEBAR_DARK_SKINS[0].value;
      } else {
        state.sidebarSkin = payload || SIDEBAR_LIGHT_SKINS[0].value;
      }
    },
    setWindowSize: (state, { payload }) => {
      state.screenSize = payload;
    },
  },
});

export const {
  toggleSidebarMenu,
  setWindowSize,
  toggleControlSidebar,
  toggleDarkMode,
  toggleBlueMode,
  setNavbarVariant,
  setSidebarSkin,
  toggleHeaderBorder,
  toggleHeaderFixed,
  toggleFooterFixed,
  toggleLayoutBoxed,
  toggleMenuItemFlat,
  toggleMenuChildIndent,
  toggleLayoutFixed,
} = uiSlice.actions;

export default uiSlice.reducer;
